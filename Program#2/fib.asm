...
prevFibo  DWORD 1     ;previous term in sequence
fiboCount DWORD ?     ;counts number of terms

.code
main PROC

;print program title and programmers name
...
;beginning of data post-test validation loop
validationLoop:
  ;get data from user
...
;Calculate Fibonnaci numbers and display Fibonacci numbers
  ...
  call  WriteDec        ;output the 1st term of the sequence
  jmp   goodbyeBlock
  ...
  fiboLoop:
  mov   eax, currentFibo    ;place current term in eax
  mov   ebx, currentFibo    ;place current term in ebx for holding
  add   eax, prevFibo     ;add previous term to current term
  call  WriteDec        ;print newest term in the sequence: current+previous 
  mov   edx, OFFSET spaces
  call  WriteString       ;print spacing between terms
  mov   currentFibo, eax    ;move newest term to current
  mov   prevFibo, ebx     ;move current term placed in ebx to previous
...
newLine:
  call  CrLf
  mov   fiboCount, 1      ;reset terms per line counter

resume:
  loop  fiboLoop        ;repeat loop if counter has not reached zero
  
;print goodbye to the user
goodbyeBlock:
  call  CrLf
  mov  edx, OFFSET certified
  call  WriteString
  call  CrLf
  mov   edx, OFFSET goodbye_1 ;"print Goodbye "
  call  WriteString
  mov   edx, OFFSET userName
  call  WriteString       
  call  CrLf
